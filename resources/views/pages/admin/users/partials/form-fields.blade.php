<div class="space-y-4">
    ...
    @if ($form->requiresAssignment())
        <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
            <h3 class="mb-3 text-sm font-semibold text-zinc-700 dark:text-zinc-200">Academic Assignment</h3>

            <div class="grid gap-4 md:grid-cols-3">
                <x-select.styled label="Campus" wire:model.live="form.campus_id" :options="$this->campuses"
                    select="label:label|value:value" :required="$form->requiresAssignment()" />
    ...
    @if ($form->requiresFacultyProfile())
        <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
            <h3 class="mb-3 text-sm font-semibold text-zinc-700 dark:text-zinc-200">Faculty Details</h3>
    ...
    @if ($form->requiresEmployeeProfile())
        <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
            <h3 class="mb-3 text-sm font-semibold text-zinc-700 dark:text-zinc-200">Employee Details</h3>

            <x-input label="Position" wire:model="form.position" :required="$form->requiresEmployeeProfile()" />
        </div>
    @endif
</div>
