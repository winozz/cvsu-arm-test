...
new class extends Component
{
    ...
    public function save(): void
    {
        ...
        try {
            ...
            DB::transaction(function (): void {
                $this->user->update($this->form->userAttributes());
                $this->user->syncRoles($this->form->normalizedRoles());
                $this->user->syncPermissions($this->form->resolvedDirectPermissions());

                $this->syncProfiles($this->user);
            });
    ...
    public function assignmentPath(): string
    {
        ...
        $path = collect([$profile->campus?->name, $profile->college?->name, $profile->department?->name])
            ->filter()
            ->implode(' / ');
    ...
    {
        $this->user = $user->load(['roles', 'permissions', 'facultyProfile.campus', 'facultyProfile.college', 'facultyProfile.department', 'employeeProfile.campus', 'employeeProfile.college', 'employeeProfile.department']);
...
<div class="space-y-6 py-8">
    <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div class="flex gap-2">
            ...
            <x-button tag="a" href="{{ route('users.index') }}" outline text="Back" sm />
        </div>
    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <p class="mt-2 text-sm text-zinc-700 dark:text-zinc-100">
                                    {{ $form->profileTypeLabel() }}</p>
                            </div>

                            <div>
                                <p class="text-xs font-medium uppercase tracking-wide text-zinc-400">Assignment</p>
                                <p class="mt-2 text-sm text-zinc-700 dark:text-zinc-100">{{ $this->assignmentPath() }}
                                </p>
                            </div>
                        </div>
                        ...
                        @if ($user->facultyProfile)
                            <div class="space-y-4 border-t border-zinc-200 pt-6 dark:border-zinc-700">
                                <div>
                                    <h3 class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">Faculty Profile
                                    </h3>
                                    <p class="text-sm text-zinc-500 dark:text-zinc-400">
                                ...
                                <div class="grid gap-6 md:grid-cols-2">
                                    <div>
                                        <p class="text-xs font-medium uppercase tracking-wide text-zinc-400">Academic
                                            Rank</p>
                                        <p class="mt-2 text-sm text-zinc-700 dark:text-zinc-100">
                                    ...
                                    <div>
                                        <p class="text-xs font-medium uppercase tracking-wide text-zinc-400">Contact
                                            Number</p>
                                        <p class="mt-2 text-sm text-zinc-700 dark:text-zinc-100">
                                    ...
                                    <div>
                                        <p class="text-xs font-medium uppercase tracking-wide text-zinc-400">Birthday
                                        </p>
                                        <p class="mt-2 text-sm text-zinc-700 dark:text-zinc-100">
                        ...
                        @if ($user->employeeProfile)
                            <div class="space-y-4 border-t border-zinc-200 pt-6 dark:border-zinc-700">
                                <div>
                                    <h3 class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">Employee Profile
                                    </h3>
                                    <p class="text-sm text-zinc-500 dark:text-zinc-400">
