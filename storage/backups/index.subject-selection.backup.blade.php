...
new class extends Component {
    ...
    #[Computed]
    public function outgoingScheduleOptions(): array
    {
        return Schedule::query()
            ...
            ->map(function (Schedule $schedule): array {
                $section = $schedule->sections->first()?->computed_section_name ?? 'No section';

                return [
                    'label' => $schedule->sched_code . ' | ' . $section . ' | ' . $schedule->subject?->code,
    ...
    public function createRequest(): void
    {
        ...
        $this->reset(['requestScheduleIds', 'requestServicingCollegeId']);
        unset($this->outgoingRequests);
        $this->toast()->success('Submitted', 'Service request submitted successfully.')->send();
    }
    ...
    public function acceptRequest(int $serviceRequestId): void
    {
        ...
        $this->toast()->success('Accepted', 'Service request accepted.')->send();
    }
    ...
    public function rejectRequest(int $serviceRequestId): void
    {
        ...
        unset($this->incomingRequests);
        $this->toast()->success('Rejected', 'Service request rejected.')->send();
    }
    ...
    public function assignToDepartment(): void
    {
        ...
        $this->toast()->success('Delegated', 'Service request delegated to department.')->send();
    }
    ...
    public function completeRequest(int $serviceRequestId): void
    {
        ...
        unset($this->incomingRequests);
        $this->toast()->success('Completed', 'Service request verified and completed.')->send();
    }
...
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-bold dark:text-white">Incoming Service Requests</h1>
    ...
    <x-card>
        <div class="space-y-4">
            <div>
                <h2 class="text-lg font-semibold dark:text-white">Create Service Request</h2>
                <p class="text-sm text-zinc-500 dark:text-zinc-400">Send a schedule to another college for plotting
                    support.</p>
            </div>

            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <x-select.styled label="Schedules (multi-select)" wire:model="requestScheduleIds" :options="$this->outgoingScheduleOptions"
            ...
            <div class="flex justify-end">
                <x-button color="primary" text="Submit Service Request" wire:click="createRequest" />
            </div>
        </div>
    </x-card>
